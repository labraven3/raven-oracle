    ...
    <nav className="sticky top-0 z-50 border-b border-white/10 bg-black/40 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        <Link href="/" className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 text-lg font-black shadow-lg shadow-violet-500/50">R</div>
          <div>
            <div className="font-black text-sm tracking-[.2em]">RAVEN</div>
            <div className="text-[8px] tracking-[.15em] text-violet-300">ORACLE</div>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {isLoggedIn ? (
            <>
              ...
              <Link href="/account" className="rounded-lg bg-gradient-to-r from-violet-500 to-purple-600 px-3 py-2 text-xs font-black shadow-lg shadow-violet-500/30">Account</Link>
              <button onClick={handleLogout} className="hidden rounded-lg border border-white/10 px-3 py-2 text-xs font-bold text-zinc-400 hover:bg-white/5 sm:block">Logout</button>
            </>
          ) : (
            <>
              <Link href="/login" className="rounded-lg border border-white/10 px-4 py-2 text-xs font-bold hover:bg-white/5">Login</Link>
              <Link href="/register" className="rounded-lg bg-gradient-to-r from-violet-500 to-purple-600 px-4 py-2 text-xs font-black shadow-lg shadow-violet-500/30">Sign Up</Link>
            </>
          )}
        </div>
      </div>
