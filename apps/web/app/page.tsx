...
function Showcase({ project }: { project?: Project }) {
  return <div className="relative min-h-[390px] overflow-hidden rounded-[30px] border border-violet-400/15 bg-[#090711] sm:min-h-[470px]">
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_48%,rgba(124,58,237,.35),transparent_38%),radial-gradient(circle_at_78%_15%,rgba(34,211,238,.12),transparent_28%)]" />
    ...
    <div className="absolute right-5 top-5 z-20 grid h-12 w-12 place-items-center rounded-2xl border border-violet-300/20 bg-white/[.05] text-violet-200 shadow-[0_0_30px_rgba(139,92,246,.25)] sm:right-7 sm:top-7 sm:h-14 sm:w-14">✦</div>
    <div className="absolute inset-x-0 top-16 flex justify-center sm:top-20"><div className="relative h-[250px] w-[250px] rotate-[-2deg] rounded-[30px] border border-violet-300/30 bg-gradient-to-br from-violet-500/20 via-[#171225] to-cyan-500/10 p-4 shadow-[0_0_80px_rgba(139,92,246,.28)] sm:h-[330px] sm:w-[330px] sm:p-5">
      <div className="absolute inset-3 rounded-[24px] border border-white/10 bg-[#5f8390] p-3 sm:inset-4 sm:p-4">{project?.logoUrl ? <img src={project.logoUrl} alt="" className="h-full w-full rounded-[18px] object-cover" /> : <div className="grid h-full w-full place-items-center rounded-[18px] bg-gradient-to-br from-violet-300/80 via-slate-700 to-cyan-300/40 text-7xl font-black text-[#12091f] sm:text-8xl">R</div>}</div>
      <div className="absolute bottom-7 left-7 rounded-full border border-white/10 bg-black/60 px-3 py-1.5 text-[8px] font-black tracking-[.18em] text-white backdrop-blur-xl sm:bottom-9 sm:left-9">{project?.name ? project.name.toUpperCase() : "FEATURED NFT"}</div>
    </div></div>
    <div className="absolute bottom-5 left-5 z-20 flex items-center gap-3 rounded-2xl border border-white/10 bg-black/55 px-3 py-2.5 backdrop-blur-xl sm:bottom-7 sm:left-7 sm:px-4 sm:py-3"><div className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br from-cyan-300 to-violet-500 text-xs font-black text-white">◈</div><div><div className="text-[7px] font-black tracking-[.2em] text-zinc-500">ECOSYSTEM</div><div className="text-xs font-black text-white">{project?.name || "Raven Oracle"}</div></div></div>
  </div>;
}
...
export default function Home() {
  ...
  return <main className={dark ? "min-h-screen overflow-x-hidden bg-[#050507] text-white" : "min-h-screen overflow-x-hidden bg-[#f7f7fb] text-zinc-950"}>
    ...
    <section className="border-y border-white/[.05] bg-white/[.012]"><div className="mx-auto w-full max-w-[1380px] px-5 py-14 sm:px-8 lg:px-10 lg:py-20"><div className="mb-8 flex items-end justify-between gap-4"><div><div className="text-[9px] font-black tracking-[.24em] text-cyan-400">RAFFLES</div><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">What's happening</h2></div><div className="flex rounded-xl border border-white/10 bg-white/[.02] p-1"><button onClick={()=>setTab("trending")} className={`rounded-lg px-4 py-2 text-[10px] font-black transition ${tab === "trending" ? "bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white" : "text-zinc-500"}`}>TRENDING</button><button onClick={()=>setTab("upcoming")} className={`rounded-lg px-4 py-2 text-[10px] font-black transition ${tab === "upcoming" ? "bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white" : "text-zinc-500"}`}>UPCOMING</button></div></div><div className="grid gap-4 lg:grid-cols-3">{loading ? [0,1,2].map((i)=><div key={i} className="h-56 animate-pulse rounded-2xl border border-white/[.06] bg-white/[.02]"/>) : displayedRaffles.slice(0,3).map((raffle)=><RaffleCard key={raffle.id} raffle={raffle}/>)}{!loading && displayedRaffles.length === 0 && <div className="col-span-full rounded-2xl border border-dashed border-white/10 py-16 text-center text-sm text-zinc-500">{tab === "trending" ? "No live raffles right now." : "No upcoming raffles right now."}</div>}</div></div></section>

    <section className="mx-auto w-full max-w-[1380px] px-5 py-8 sm:px-8 lg:px-10"><div className="overflow-hidden rounded-2xl border border-white/[.07] bg-white/[.018]"><div className="flex min-w-max items-center gap-10 px-5 py-4 text-[9px] font-bold text-zinc-400"><span><b className="text-violet-400">@0xRaven</b> entered a raffle</span><span><b className="text-violet-400">@AlphaHunter</b> won a WL spot</span><span><b className="text-violet-400">@Web3Legend</b> entered a raffle</span><span><b className="text-violet-400">@MoonBag</b> won 3 WL spots</span></div></div></section>
  </main>;
}
