  ...
  return (
    <main style={{ minHeight: "100vh", background: colors.bg, color: colors.text, display: "flex", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      <aside style={{ width: 220, flexShrink: 0, minHeight: "100vh", background: colors.sidebar, borderRight: `1px solid ${colors.border}`, padding: "22px 14px", position: "sticky", top: 0, height: "100vh", display: "flex", flexDirection: "column" }}>
        <a href="/" style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 8px 24px", color: colors.text }}>
        ...
        <nav style={{ display: "grid", gap: 3 }}>
          <a href="/" style={nav(true)}>⌂ <span>Home</span></a>
          <a href="/dashboard" style={nav()}>▦ <span>Creator Studio</span></a>
          <a href="/account" style={nav()}>◎ <span>Account</span></a>
        </nav>
        <div style={{ margin: "22px 7px 9px", color: colors.muted, fontSize: 8, letterSpacing: ".16em", fontWeight: 900 }}>EXPLORE</div>
        <nav style={{ display: "grid", gap: 3 }}>
          <a href="/projects" style={nav()}>◈ <span>NFT Projects</span></a>
          <a href="/raffles" style={nav()}>🎟 <span>Raffles</span></a>
          <a href="/alpha" style={nav()}>♛ <span>King of Alpha</span></a>
          <a href="/how-it-works" style={nav()}>ⓘ <span>How it works</span></a>
        </nav>
        <div style={{ marginTop: "auto" }}>
          ...
          <a href="/account" style={{ display: "block", marginTop: 8, textAlign: "center", background: colors.text, color: theme === "dark" ? "#0b0a12" : "white", borderRadius: 8, padding: 10, fontSize: 11, fontWeight: 900 }}>Account →</a>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <header style={{ height: 64, borderBottom: `1px solid ${colors.border}`, display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "0 28px", gap: 18, background: colors.bg }}>
          ...
          <a href="/projects/new" style={{ border: `1px solid ${colors.border}`, background: colors.panel, color: colors.text, borderRadius: 8, padding: "9px 13px", fontSize: 10, fontWeight: 800 }}>Create project</a>
        </header>
        ...
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "28px 28px 60px" }}>
          <section style={{ display: "flex", alignItems: "end", justifyContent: "space-between", gap: 20, marginBottom: 18 }}>
            ...
            <a href="/raffles" style={{ color: colors.accent, fontSize: 11, fontWeight: 900 }}>View all raffles →</a>
          </section>
          ...
          <section style={{ marginBottom: 30 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><h2 style={{ fontSize: 15, margin: 0 }}>Trending projects</h2><a href="/projects" style={{ color: colors.muted, fontSize: 10 }}>View all →</a></div>
            {loading ? <div style={{ ...card, padding: 28, color: colors.muted, fontSize: 11 }}>Loading projects…</div> : trendingProjects.length === 0 ? <div style={{ ...card, padding: 28, color: colors.muted, fontSize: 11 }}>No approved NFT projects yet.</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 10 }}>{trendingProjects.map((p) => <a href={`/projects/${p.id}`} key={p.id} style={{ ...card, padding: 13, minWidth: 0 }}><div style={{ display: "flex", alignItems: "center", gap: 9 }}><div style={{ width: 34, height: 34, flexShrink: 0, borderRadius: 8, background: colors.panel2, display: "grid", placeItems: "center", color: colors.accent, fontWeight: 900, fontSize: 10, overflow: "hidden" }}>{p.logoUrl ? <img src={p.logoUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : initials(p.name)}</div><div style={{ minWidth: 0 }}><b style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11 }}>{p.name}</b><span style={{ color: colors.muted, fontSize: 9 }}>NFT · {p.status ?? "APPROVED"}</span></div></div><div style={{ marginTop: 14, display: "flex", justifyContent: "space-between", color: colors.muted, fontSize: 9 }}><span>Raffles</span><span style={{ color: colors.text }}>Explore →</span></div></a>)}</div>}
          </section>

          {featured && <section style={{ marginBottom: 30 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}><h2 style={{ fontSize: 15, margin: 0 }}>Raven Oracle pick</h2><span style={{ color: colors.green, fontSize: 9, fontWeight: 900 }}>APPROVED</span></div><a href={`/projects/${featured.id}`} style={{ ...card, padding: 20, display: "grid", gridTemplateColumns: "1fr 230px", gap: 20, background: `linear-gradient(120deg,${colors.panel},${colors.panel2})` }}><div><div style={{ color: colors.accent, fontSize: 9, letterSpacing: ".15em", fontWeight: 900 }}>FEATURED NFT PROJECT</div><h2 style={{ margin: "9px 0 7px", fontSize: 24 }}>{featured.name}</h2><p style={{ color: colors.muted, fontSize: 11, lineHeight: 1.7, maxWidth: 650 }}>{featured.description || "Explore this approved NFT community and its upcoming whitelist opportunities."}</p><span style={{ display: "inline-block", marginTop: 12, color: colors.text, fontSize: 10, fontWeight: 900 }}>View project →</span></div><div style={{ minHeight: 120, borderRadius: 10, background: colors.bg, border: `1px solid ${colors.border}`, display: "grid", placeItems: "center", overflow: "hidden" }}>{featured.logoUrl ? <img src={featured.logoUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 34, fontWeight: 900, color: colors.accent }}>{initials(featured.name)}</span>}</div></a></section>}

          <section style={{ marginBottom: 30 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><div><h2 style={{ fontSize: 15, margin: 0 }}>Trending raffles</h2><span style={{ color: colors.muted, fontSize: 9 }}>Live and scheduled whitelist opportunities</span></div><a href="/raffles" style={{ color: colors.muted, fontSize: 10 }}>View all →</a></div>
            {loading ? <div style={{ ...card, padding: 28, color: colors.muted, fontSize: 11 }}>Loading raffles…</div> : trendingRaffles.length === 0 ? <div style={{ ...card, padding: 28, color: colors.muted, fontSize: 11 }}>No active raffles yet.</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 10 }}>{trendingRaffles.map((r) => <a href={`/raffles/${r.id}`} key={r.id} style={{ ...card, padding: 14 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}><span style={{ color: colors.green, fontSize: 8, fontWeight: 900 }}>{r.status}</span><span style={{ color: colors.muted, fontSize: 9 }}>{timeLabel(r.endsAt)}</span></div><b style={{ display: "block", marginTop: 16, fontSize: 13 }}>{r.title}</b><span style={{ display: "block", marginTop: 5, color: colors.muted, fontSize: 9 }}>{r.project?.name ?? "NFT Project"}</span><div style={{ marginTop: 16, paddingTop: 11, borderTop: `1px solid ${colors.border}`, display: "flex", justifyContent: "space-between", fontSize: 9 }}><span style={{ color: colors.muted }}>Prize</span><strong>{r.prizeQuantity} × {r.prizeName}</strong></div></a>)}</div>}
          </section>

          <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ ...card, padding: 18 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 15 }}><h2 style={{ fontSize: 14, margin: 0 }}>King of Alpha</h2><a href="/alpha" style={{ color: colors.muted, fontSize: 9 }}>Leaderboard →</a></div>{leaders.length === 0 ? <span style={{ color: colors.muted, fontSize: 10 }}>No leaderboard data yet.</span> : leaders.map((l, i) => <div key={l.userId} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderTop: i ? `1px solid ${colors.border}` : "none" }}><span style={{ width: 20, color: colors.muted, fontSize: 9 }}>#{i + 1}</span><div style={{ flex: 1 }}><b style={{ fontSize: 10 }}>{l.displayName || l.username || "Anonymous"}</b></div><strong style={{ color: colors.accent, fontSize: 10 }}>{l.points} pts</strong></div>)}</div>
            <div style={{ ...card, padding: 18 }}><h2 style={{ fontSize: 14, margin: "0 0 15px" }}>For creators</h2><p style={{ color: colors.muted, fontSize: 10, lineHeight: 1.7, margin: 0 }}>Launch an NFT whitelist raffle, configure eligibility tasks, draw verified winners and export the final whitelist.</p><div style={{ display: "flex", gap: 8, marginTop: 18 }}><a href="/projects/new" style={{ background: colors.accent, color: "white", borderRadius: 7, padding: "9px 12px", fontSize: 9, fontWeight: 900 }}>Create project</a><a href="/dashboard" style={{ border: `1px solid ${colors.border}`, color: colors.text, borderRadius: 7, padding: "9px 12px", fontSize: 9, fontWeight: 800 }}>Creator Studio</a></div></div>
          </section>
        </div>
      </div>
    </main>
