...
export default function CreateRaffle() {
  ...
  const create = async () => {
    if (!projectId) return setMessage("Select a project before creating the raffle.");
    ...
    if (tasks.some((task) => !task.title.trim() || !task.target.trim())) {
    ...
    setMessage("");
    try {
      const raffle = await api<{ raffle: { id: string } }>("/raffles/", {
        method: "POST",
        body: JSON.stringify({
          projectId,
          title: title.trim(),
    ...
    <main className="min-h-screen bg-[#07070a] text-zinc-100">
      ...
      <div className="mx-auto max-w-6xl px-5 py-12">
        <div className="max-w-3xl">
          <span className="text-[9px] font-black tracking-[.2em] text-violet-300/60">CREATOR STUDIO</span>
          <h1 className="mt-3 text-5xl font-medium tracking-tight">Create a raffle.</h1>
          <p className="mt-4 text-sm leading-6 text-zinc-500">
            Choose the project, define the prize and schedule, then add the exact social actions participants must complete.
          </p>
        </div>

        <section className="mt-10 rounded-2xl border border-white/10 bg-[#0d0c11] p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <label className="flex-1">Project
              <select value={projectId} disabled={loadingProjects} onChange={(e) => setProjectId(e.target.value)}>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          <section className="rounded-2xl border border-white/10 bg-[#0d0c11] p-6">
            ...
            <div className="mt-5 grid gap-4">
              <label>Title<input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="XYZ Community Raffle" /></label>
              <label>Description<textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Tell participants what they can win and why." /></label>
              <div className="grid gap-4 sm:grid-cols-2">
                <label>Prize<input value={prizeName} onChange={(e) => setPrizeName(e.target.value)} placeholder="1× WL Spot" /></label>
                <label>Prize details<input value={prizeDescription} onChange={(e) => setPrizeDescription(e.target.value)} placeholder="Whitelist allocation" /></label>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <label>Prize quantity<input type="number" min={1} value={quantity} onChange={(e) => setQuantity(Math.max(1, Number(e.target.value)))} /></label>
          ...
          <aside className="rounded-2xl border border-white/10 bg-[#0d0c11] p-6">
            <span className="text-[9px] font-black tracking-[.2em] text-zinc-600">PUBLISH CHECKLIST</span>
            <div className="mt-5 space-y-3 text-sm text-zinc-500">
              <div>✓ Project attached</div>
              <div>✓ Prize configured</div>
              <div>✓ Entry window configured</div>
              <div>✓ Required tasks configured</div>
              <div>✓ Automated verification</div>
              <div>✓ Deterministic fair draw</div>
            </div>
          </aside>
        </div>

        <section className="mt-5 rounded-2xl border border-white/10 bg-[#0d0c11] p-6">
          <div className="flex items-center justify-between">
            <div><span className="text-[9px] font-black tracking-[.2em] text-zinc-600">ELIGIBILITY TASKS</span><h2 className="mt-2 text-lg font-bold">What participants must do</h2></div>
            <button onClick={addTask} className="rounded-lg border border-white/10 px-3 py-2 text-xs font-bold">+ Add task</button>
          </div>
            ...
            {tasks.map((task, index) => (
              <div key={index} className="rounded-xl border border-white/10 bg-black/20 p-4">
                <div className="grid gap-3 md:grid-cols-[160px_1fr_1fr]">
                  <select value={task.type} onChange={(e) => updateTask(index, { type: e.target.value as TaskDraft["type"] })}>
                    <option value="X_FOLLOW">X Follow</option><option value="X_LIKE">X Like</option><option value="X_REPOST">X Repost</option><option value="DISCORD_JOIN">Discord Join</option>
                  </select>
                  <input value={task.title} onChange={(e) => updateTask(index, { title: e.target.value })} placeholder="Task title" />
                  <input value={task.target} onChange={(e) => updateTask(index, { target: e.target.value })} placeholder={task.type === "DISCORD_JOIN" ? "Discord server ID" : "X username / post ID"} />
                </div>
                <div className="mt-3 flex gap-3">
                  <input className="flex-1" value={task.targetUrl} onChange={(e) => updateTask(index, { targetUrl: e.target.value })} placeholder="Target URL" />
                  <label className="flex items-center gap-2 text-xs text-zinc-500"><input type="checkbox" checked={task.isRequired} onChange={(e) => updateTask(index, { isRequired: e.target.checked })} /> Required</label>
                  {tasks.length > 1 && <button onClick={() => setTasks((items) => items.filter((_, i) => i !== index))} className="rounded border border-red-900/40 px-3 text-xs text-red-400">Remove</button>}
                </div>
              </div>
            ))}
          </div>
        ...
        <button disabled={busy || loadingProjects} onClick={() => void create()} className="mt-6 w-full rounded-xl bg-violet-500 py-4 text-sm font-black shadow-lg shadow-violet-500/10 disabled:cursor-not-allowed disabled:opacity-50">
