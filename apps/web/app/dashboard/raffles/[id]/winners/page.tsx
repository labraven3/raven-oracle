...
export default function WinnerCenterPage() {
  ...
  const copyWinnerData = async () => {
    setError(""); setMessage("");
    try {
      const header = ["Rank", "Winner", "Wallet Address", "Email", "Status", "Notification Status"];
      const rows = winnerData.map(w => [String(w.rank), w.name, w.wallet, w.email, w.status, w.notification]);
      const text = [header, ...rows].map(row => row.map(value => `"${String(value).replace(/"/g, '""')}"`).join("\t")).join("\n");
      await navigator.clipboard.writeText(text);
      setMessage("Winner data copied. Open your Google Sheet and paste it.");
    } catch {
      setError("Could not copy winner data. Please use Export CSV instead.");
    }
  };
