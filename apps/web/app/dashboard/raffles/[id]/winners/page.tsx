...
async function api<T>(path: string, options: RequestInit = {}) {
  ...
  const response = await fetch(`${API_BASE_URL}${path}`, { ...options, headers, credentials: "include", cache: "no-store" }); const data = await response.json().catch(() => ({})); if (!response.ok) { const error = new Error(data.message ?? `Request failed (${response.status})`); (error as Error & { code?: string }).code = data.code; throw error; } return data as T;
}
...
export default function WinnerCenterPage() {
  ...
  const [raffle, setRaffle] = useState<Raffle | null>(null); const [winners, setWinners] = useState<Winner[]>([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(""); const [error, setError] = useState(""); const [message, setMessage] = useState("");
  const load = async () => { setLoading(true); setError(""); try { const data = await api<{ raffle: Raffle; winners: Winner[] }>(`/raffles/${id}/winners`); setRaffle(data.raffle); setWinners(data.winners ?? []); } catch (e) { setError(e instanceof Error ? e.message : "Unable to load Winner Center"); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, [id]);
  const notify = async (winnerId: string, resend = false) => { setBusy(winnerId); setError(""); setMessage(""); try { await api(`/raffles/${id}/winners/${winnerId}/${resend ? "resend" : "notify"}`, { method: "POST" }); setMessage(resend ? "Winner email resent successfully." : "Winner email sent successfully."); await load(); } catch (e) { setError(e instanceof Error ? e.message : "Notification failed"); } finally { setBusy(""); } };
  const downloadWinners = async () => {
    setBusy("export"); setError(""); setMessage("");
    try {
      const headers = new Headers(); const token = localStorage.getItem("raven_token"); if (token) headers.set("Authorization", `Bearer ${token}`);
      ...
      const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `raven-oracle-${id}-winners.xlsx`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); setMessage("Winner spreadsheet downloaded successfully.");
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to export winners"); } finally { setBusy(""); }
  };
  ...
  return <main className="min-h-screen bg-[#06060a] text-zinc-100"><SiteHeader/><div className="mx-auto max-w-6xl px-5 py-8">
    <div className="flex flex-wrap items-center justify-between gap-3"><Link href={`/dashboard/raffles/${id}`} className="text-xs text-zinc-500 hover:text-violet-300">← Raffle operations</Link><button type="button" onClick={()=>void downloadWinners()} disabled={busy==="export"||raffle?.status!=="COMPLETED"||winners.length===0} className="rounded-xl bg-emerald-500 px-4 py-2.5 text-xs font-black text-black disabled:cursor-not-allowed disabled:opacity-40">{busy==="export"?"Preparing spreadsheet…":"Export winners (.xlsx)"}</button></div>
    <section className="mt-5 rounded-3xl border border-white/10 bg-[#0d0c11] p-7"><div className="flex flex-wrap items-start justify-between gap-5"><div><span className="text-[9px] font-black tracking-[.2em] text-violet-300/60">WINNER CENTER</span><h1 className="mt-2 text-4xl font-semibold">{raffle?.title}</h1><p className="mt-2 text-sm text-zinc-500">{raffle?.prizeName} · {winners.length} / {raffle?.winnerCount ?? 0} winners</p><p className="mt-2 text-xs text-zinc-600">Raffle ended: {formatDate(raffle?.endsAt)}</p></div><div className="max-w-md rounded-2xl border border-emerald-500/15 bg-emerald-500/5 p-4"><div className="text-[9px] font-black tracking-[.18em] text-emerald-300">WINNER EXPORT</div><p className="mt-2 text-xs leading-5 text-zinc-400">Download a normal Excel spreadsheet with winner rank, X username, Discord username, wallet address, email, notification status, and task verification details.</p></div></div></section>
