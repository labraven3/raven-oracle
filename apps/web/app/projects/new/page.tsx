...
export default function NewProject() {
  ...
  const handleLogo = (file: File | undefined) => { setMessage(""); if (!file) return; if (!ACCEPTED_LOGO_TYPES.includes(file.type)) return setMessage("Logo must be PNG, JPG or WEBP."); if (file.size > MAX_LOGO_BYTES) return setMessage("Logo must be 2 MB or smaller."); const reader = new FileReader(); reader.onload = () => { if (typeof reader.result !== "string") return setMessage("Unable to read that image."); set("logoUrl", reader.result); setLogoName(file.name); }; reader.onerror = () => setMessage("Unable to read that image."); reader.readAsDataURL(file); };
  const submit = async () => {
    ...
    try {
      ...
      setMessage(`Project “${data.project.name}” submitted for approval.`);
      setTimeout(() => router.push(`/dashboard/projects/${data.project.id}`), 400);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to create project"); } finally { setBusy(false); }
  };
  return (
    <main className="min-h-screen bg-[#07070a] text-zinc-100">
      ...
      <div className="mx-auto max-w-5xl px-5 py-14">
        <div className="max-w-2xl"><span className="text-[9px] font-black tracking-[.2em] text-violet-300/60">PROJECT ONBOARDING</span><h1 className="mt-3 text-5xl font-medium tracking-tight">Create a project.</h1><p className="mt-4 text-sm leading-6 text-zinc-500">Choose NFT, Token, Game, Tool, DeFi, Community or another category. After approval, the project dashboard is where its raffles are published.</p></div>
        <section className="mt-10 rounded-2xl border border-white/10 bg-[#0d0c11] p-6"><div className="grid gap-5 md:grid-cols-2">
          ...
          <label>Category <span className="text-violet-300">*</span><select value={form.category} onChange={(e) => set("category", e.target.value)}>{["NFT","TOKEN","GAME","TOOL","DEFI","COMMUNITY","OTHER"].map((item) => <option key={item}>{item}</option>)}</select></label>
