  ...
  return <main className="min-h-screen bg-[#07070a] text-zinc-100">
    ...
    <div className="mx-auto max-w-5xl px-5 py-12 sm:py-14">
      ...
      <section className="mt-9 rounded-3xl border border-white/10 bg-[#0d0c11] p-6 sm:p-7">
        ...
        <div className="mt-6 rounded-2xl border border-violet-500/15 bg-violet-500/[.03] p-5"><div className="text-[9px] font-black tracking-[.2em] text-violet-300/70">COLLECTION DETAILS</div><p className="mt-2 text-xs leading-5 text-zinc-600">Optional collection information used to describe the project accurately.</p><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="text-xs text-zinc-400">Collection supply <span className="text-zinc-600">(Optional)</span><input type="number" min="1" step="1" value={nftMeta.supply} onChange={(e) => setNftMeta((current) => ({ ...current, supply: e.target.value }))} placeholder="7000" /></label><label className="text-xs text-zinc-400">Mint date <span className="text-zinc-600">(Optional)</span><input value={nftMeta.mintDate} onChange={(e) => setNftMeta((current) => ({ ...current, mintDate: e.target.value }))} placeholder="TBD" /><span className="mt-1 block text-[10px] text-zinc-600">Use TBD if the mint date has not been announced.</span></label></div></div>
