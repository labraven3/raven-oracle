...
async function adminApi<T>(path: string, options: RequestInit = {}) {
...
export default function NewProject() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/chains`, { cache: "no-store" })
      ...
      .then((data) => { setChains(data.chains ?? []); if (data.chains?.[0]) setForm((current) => ({ ...current, chain: current.chain || data.chains![0].name })); })
  ...
  const submit = async () => {
    ...
    try {
      ...
      const data = await adminApi<{ project: { id: string; name: string } }>("/projects/", { method: "POST", body: JSON.stringify(payload) });
      await adminApi(`/project-metadata/${data.project.id}`, { method: "PUT", body: JSON.stringify({ projectType: "NFT", metadata: { supply: nftMeta.supply ? Number(nftMeta.supply) : undefined, mintDate: nftMeta.mintDate.trim() || "TBD" } }) });
      ...
      if (/authentication|admin|unauthorized/i.test(msg)) router.replace("/admin/login");
      setMessage(msg);
    } finally { setBusy(false); }
  };

  if (!authorized) return <main className="min-h-screen bg-[#07070a] text-zinc-100"><div className="mx-auto flex min-h-screen max-w-xl items-center justify-center px-5"><div className="text-center"><div className="text-[9px] font-black tracking-[.2em] text-violet-300/70">ADMIN ONLY</div><h1 className="mt-3 text-3xl font-medium">Checking access…</h1></div></div></main>;

  return <main className="min-h-screen bg-[#07070a] text-zinc-100">
    <header className="border-b border-white/10 px-5 py-5"><div className="mx-auto flex max-w-5xl items-center justify-between"><Link href="/admin" className="font-black tracking-[.18em]">RAVEN ORACLE · ADMIN</Link><Link href="/admin" className="text-xs text-zinc-500 hover:text-zinc-300">Back to Admin →</Link></div></header>
    <div className="mx-auto max-w-5xl px-5 py-12 sm:py-14">
      <div className="max-w-2xl"><span className="text-[9px] font-black tracking-[.2em] text-violet-300/60">ADMIN · NFT PROJECT</span><h1 className="mt-3 text-4xl font-medium tracking-tight sm:text-5xl">Add an NFT project.</h1><p className="mt-4 text-sm leading-6 text-zinc-500">Create the public NFT listing used across Raven Oracle.</p></div>
      ...
      <button disabled={busy} onClick={() => void submit()} className="mt-6 w-full rounded-2xl bg-violet-600 py-4 text-sm font-black text-white hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Creating…" : "Create NFT project"}</button>
    </div>
  </main>;
}
