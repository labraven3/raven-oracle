    ...
    <main className="min-h-screen bg-[#f6f7f9] text-[#111827] dark:bg-[#07070a] dark:text-zinc-100">
      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-4 py-5 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 border-t border-black/5 pt-4 dark:border-white/5">
            ...
            <div className="space-y-1">
              {categories.slice(1).map((item) => (
                <button key={item} onClick={() => setCategory(item)} className={`block w-full rounded-lg px-3 py-2 text-left text-[11px] ${category === item ? "bg-violet-500 text-white" : "text-zinc-500 hover:bg-black/5 dark:hover:bg-white/5"}`}>
                  {item}
                </button>
              ))}
            </div>
        ...
        <section className="min-w-0 flex-1">
          <div className="rounded-3xl border border-black/5 bg-white p-5 sm:p-7 dark:border-white/10 dark:bg-[#0d0c11]">
            ...
            <div className="mt-7 flex flex-col gap-3 lg:flex-row">
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((item) => (
                  <button key={item} onClick={() => setCategory(item)} className={`rounded-xl border px-3.5 py-2.5 text-[10px] font-black ${category === item ? "border-violet-500/30 bg-violet-500 text-white" : "border-black/10 bg-white text-zinc-500 dark:border-white/10 dark:bg-black/20"}`}>
                    {item}
                  </button>
                ))}
              </div>
          ...
          {loading ? (
            <div className="mt-5 rounded-2xl border border-black/5 bg-white p-14 text-center text-sm text-zinc-500 dark:border-white/10 dark:bg-[#0d0c11]">Loading projects…</div>
          ) : filtered.length === 0 ? (
            <div className="mt-5 rounded-2xl border border-dashed border-black/10 bg-white p-14 text-center dark:border-white/10 dark:bg-[#0d0c11]">
              <h2 className="text-lg font-semibold">No projects found</h2>
              <p className="mt-2 text-sm text-zinc-500">Try another search or category.</p>
            </div>
          ) : (
            <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {filtered.map((project) => (
                <Link key={project.id} href={`/projects/${project.id}`} className="group overflow-hidden rounded-2xl border border-black/5 bg-white transition hover:-translate-y-1 hover:shadow-xl dark:border-white/10 dark:bg-[#0d0c11]">
                  <div className="relative h-40 overflow-hidden bg-gradient-to-br from-violet-100 via-white to-slate-100 dark:from-violet-950/40 dark:via-[#15101e] dark:to-[#0b0b0f]">
                    {project.bannerUrl ? (
                      <img src={project.bannerUrl} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                    ) : (
                      <div className="h-full w-full bg-gradient-to-br from-violet-950/60 via-[#15101e] to-[#0b0b0f]" />
                    )}
                    <div className="absolute right-3 top-3 rounded-full border border-white/10 bg-black/40 px-2.5 py-1 text-[8px] font-black text-white backdrop-blur">{project.category}</div>
                    <div className="absolute bottom-[-1.5rem] left-5 grid h-16 w-16 place-items-center overflow-hidden rounded-2xl border-4 border-white bg-white text-xl font-black text-violet-500 shadow-sm dark:border-[#0d0c11] dark:bg-black dark:text-violet-200">
                  ...
                  <div className="p-5 pt-9">
                    <div className="flex items-start gap-3">
                      <div className="min-w-0 flex-1">
                        <h2 className="truncate text-lg font-semibold group-hover:text-violet-500 dark:group-hover:text-violet-200">{project.name}</h2>
                        <p className="mt-2 line-clamp-2 text-xs leading-5 text-zinc-500">{project.description || "Approved community on Raven Oracle."}</p>
                      </div>
