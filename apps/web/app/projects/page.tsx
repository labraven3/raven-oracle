...
export default function ProjectsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter((project) =>
      ...
      (!q || `${project.name} ${project.description ?? ""} ${project.category} ${project.chain ?? ""}`.toLowerCase().includes(q))
    );
      ...
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-5 border-b border-black/5 pb-7 dark:border-white/10 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-500">Discover verified Web3 projects, communities and products across the networks supported by Raven Oracle.</p>
          </div>
          ...
          <div className="p-5 sm:p-6">
            <div className="grid gap-3 lg:grid-cols-[1fr_230px]">
              <label className="flex items-center rounded-xl border border-black/10 bg-[#fafafa] px-4 dark:border-white/10 dark:bg-black/20">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search projects, category or chain" className="w-full bg-transparent py-3 text-sm outline-none placeholder:text-zinc-400" />
              </label>
        ...
        {loading ? (
          ...
          <div className="mt-5 rounded-2xl border border-dashed border-black/10 bg-white p-16 text-center dark:border-white/10 dark:bg-[#0d0c11]"><h2 className="text-lg font-semibold">No projects found</h2><p className="mt-2 text-sm text-zinc-500">Try another search, category or chain.</p></div>
        ) : (
            ...
            {filtered.map((project) => (
              <Link key={project.id} href={`/projects/${project.id}`} className="group overflow-hidden rounded-2xl border border-black/5 bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-lg hover:shadow-violet-500/5 dark:border-white/10 dark:bg-[#0d0c11] dark:hover:border-violet-500/40">
                <div className="relative h-48 overflow-hidden bg-gradient-to-br from-violet-100 via-slate-100 to-slate-200 dark:from-violet-950/40 dark:via-[#17171d] dark:to-[#0a0a0d]">
                  ...
                  <div className="absolute inset-x-0 top-0 flex items-start justify-between p-4"><span className="rounded-full bg-black/45 px-3 py-1.5 text-[9px] font-black text-white backdrop-blur">{project.category}</span><span className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-1.5 text-[9px] font-black text-emerald-200 backdrop-blur">VERIFIED</span></div>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-black/5 pt-4 text-[9px] font-black uppercase tracking-[.12em] text-zinc-400 dark:border-white/5"><span>Project</span><span className="text-violet-500">View details</span></div>
                </div>
              </Link>
            ))}
          </div>
