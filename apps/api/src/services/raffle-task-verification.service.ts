...
async function verifyXFollow(userId: string, target: string, projectXUrl?: string | null): Promise<VerificationResult> {
  ...
  // One page only. The old implementation could walk 20 pages / 20,000
  // accounts for one click, which caused the $3.78 usage spike.
...
async function verifyXTweetEngagement(userId: string, taskType: "X_LIKE" | "X_REPOST", target: string): Promise<VerificationResult> {
  ...
  return data.data.some((user) => user.id === tokenData.account.providerAccountId)
...
async function verifyDiscordJoin(userId: string, target: string): Promise<VerificationResult> {
  const account = await prisma.socialAccount.findFirst({ where: { userId, provider: "DISCORD", isActive: true } });
  ...
  try { accessToken = decryptDiscordToken(account.accessTokenEncrypted); } catch { return { verified: false, reason: "Unable to decrypt Discord access token" }; }
  const raw = target.trim();
  const guildId = /^\d{5,25}$/.test(raw) ? raw : raw.match(/discord(?:app)?\.com\/channels\/(\d{5,25})/)?.[1];
  if (!guildId) return { verified: false, reason: "Discord task target must contain the server/guild ID" };
  const response = await fetch("https://discord.com/api/users/@me/guilds", { headers: { Authorization: `Bearer ${accessToken}`, Accept: "application/json" } });
  ...
  return guild ? { verified: true, evidence: { guildId, guildName: guild.name ?? null } } : { verified: false, reason: "The connected Discord account is not a member of the required server" };
}

export async function verifyRaffleTask(taskId: string, entryId: string, userId: string): Promise<VerificationResult> {
  const task = await prisma.raffleTask.findUnique({ where: { id: taskId }, include: { raffle: { select: { project: { select: { xUrl: true } } } } } });
  ...
  const result = task.type === "X_FOLLOW"
    ...
    : task.type === "X_LIKE" || task.type === "X_REPOST"
      ...
      : await verifyDiscordJoin(userId, task.target);
