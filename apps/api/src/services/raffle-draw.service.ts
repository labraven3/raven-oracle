...
export async function drawRaffle(raffleId: string, requestingUserId: string, options: DrawOptions = {}) {
  return prisma.$transaction(async (tx) => {
    ...
    const eligibleEntries = await tx.raffleEntry.findMany({ where: { raffleId, status: "ELIGIBLE", walletAddressId: { not: null }, walletAddressSnapshot: { not: null } }, orderBy: [{ enteredAt: "asc" }, { id: "asc" }], select: { id: true, userId: true, walletAddressSnapshot: true, enteredAt: true } });
    ...
    const entryIds = eligibleEntries.map((entry) => entry.id); const eligibleEntryIdsHash = hashEntryIds(entryIds);
    const randomness = crypto.randomBytes(32); const randomnessValueHash = createHash("sha256").update(randomness).digest("hex");
