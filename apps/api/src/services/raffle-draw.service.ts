...
export async function drawRaffle(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (new Date() < raffle.endsAt) {
      ...
      if (!(options.allowEarlyFcfs && isFcfs)) throw new Error("Raffle end time has not been reached");
    }

    const claimed = await tx.raffle.updateMany({ where: { id: raffleId, status: "CLOSED" }, data: { status: "DRAWING" } });
    ...
    const snapshot = await tx.raffleEligibilitySnapshot.create({
      data: { raffleId, eligibleEntryCount: eligibleEntries.length, eligibleEntryIdsHash, randomnessSource: raffleType === "FCFS" ? "entry-order" : "node:crypto.randomBytes", randomnessRequestRef: null, randomnessValueHash, algorithmVersion, winnerIndexResults: selectedIndexes },
    });
    ...
    await tx.auditLog.create({
      data: {
        ...
        metadata: { winnerCount, eligibleEntryCount: eligibleEntries.length, algorithmVersion, eligibleEntryIdsHash, randomnessSource: raffleType === "FCFS" ? "entry-order" : "node:crypto.randomBytes", randomnessValueHash, snapshotId: snapshot.id, winnerIndexes: selectedIndexes },
      },
    });
...
export async function maybeAutoDrawFcfs(raffleId: string, requestingUserId: string) {
  ...
  const eligibleCount = await prisma.raffleEntry.count({ where: { raffleId, status: "ELIGIBLE", walletAddressId: { not: null }, walletAddressSnapshot: { not: null } } });
