...
export async function drawRaffle(
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const pendingCount = await tx.raffleEntry.count({
        where: { raffleId, status: "PENDING" },
      });
      if (pendingCount > 0) {
        throw new Error(
          `Raffle has ${pendingCount} unevaluated entr${pendingCount === 1 ? "y" : "ies"}; evaluate entries before drawing`,
        );
      }
      ...
      if (eligibleEntries.length === 0) {
        throw new Error("No eligible entries with payout wallets available");
      }
      ...
      for (let rank = 1; rank <= winnerCount; rank += 1) {
        ...
        if (!selected?.entry.walletAddressSnapshot) {
          throw new Error("Winner selection failed: payout wallet missing");
        }
      ...
      // Prisma 7's generated JSON input types can be stricter than the
      // runtime JSON column accepts. Serialize/parse here to keep the
      // persisted values as JSON objects/arrays while avoiding the generated
      // client type incompatibility that was breaking production typecheck.
      ...
      await tx.auditLog.create({
        data: {
          actorUserId: options.actorUserId ?? requestingUserId,
      ...
      const updatedRaffle = await tx.raffle.update({
        ...
        data: {
          status: "COMPLETED",
          fairnessAlgorithmVersion: algorithmVersion,
        },
      });
