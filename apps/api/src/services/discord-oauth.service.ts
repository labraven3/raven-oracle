...
function encryptState(payload: { userId: string | null; createdAt: number; nonce: string }) { return encrypt(JSON.stringify(payload)); }
function decryptState(state: string) {
  const parsed = JSON.parse(decrypt(state)) as { userId: string | null; createdAt: number; nonce: string };
  if (!parsed.createdAt || !parsed.nonce || parsed.createdAt + 10 * 60 * 1000 < Date.now()) throw new Error("Discord OAuth state expired");
  return parsed;
}
...
async function getDiscordUser(accessToken: string) {
  const response = await fetch(DISCORD_ME_URL, { headers: { Authorization: `Bearer ${accessToken}` } });
  const data = await response.json() as { id: string; username?: string; global_name?: string | null; avatar?: string | null; email?: string | null; verified?: boolean };
  if (!response.ok || !data.id) throw new Error("Unable to retrieve Discord profile");
  return data;
}
...
export async function connectDiscordAccount(code: string, state: string) {
  ...
  const discordEmail = discordUser.email?.trim().toLowerCase() || null;
  const verifiedDiscordEmail = Boolean(discordEmail && discordUser.verified);
  ...
  if (!user) {
    user = await prisma.user.create({
      data: {
        email: verifiedDiscordEmail ? discordEmail : null,
        emailVerifiedAt: verifiedDiscordEmail ? new Date() : null,
        status: verifiedDiscordEmail ? "ACTIVE" : "PENDING",
  ...
  } else if (stateData.userId && user.id === stateData.userId && !user.email && verifiedDiscordEmail) {
    const conflict = await prisma.user.findUnique({ where: { email: discordEmail! } });
    if (conflict && conflict.id !== user.id) throw new Error("That Discord email is already attached to another Raven Oracle account");
    user = await prisma.user.update({ where: { id: user.id }, data: { email: discordEmail, emailVerifiedAt: new Date(), status: "ACTIVE" } });
  } else if (verifiedDiscordEmail && user.email && user.email !== discordEmail) {
    throw new Error("This Discord account uses an email already different from the Raven Oracle account email");
  }

  const account = await prisma.socialAccount.upsert({
    where: { provider_providerAccountId: { provider: "DISCORD", providerAccountId: discordUser.id } },
    create: {
      userId: user.id, provider: "DISCORD", providerAccountId: discordUser.id,
      providerUsername: discordUser.username ?? null, displayName: discordUser.global_name ?? discordUser.username ?? null,
      avatarUrl: discordAvatarUrl(discordUser.id, discordUser.avatar), accessTokenEncrypted: encrypt(token.access_token!),
      refreshTokenEncrypted: token.refresh_token ? encrypt(token.refresh_token) : null,
      tokenExpiresAt: token.expires_in ? new Date(Date.now() + token.expires_in * 1000) : null, isActive: true, disconnectedAt: null,
    },
    update: {
      userId: user.id, providerUsername: discordUser.username ?? null, displayName: discordUser.global_name ?? discordUser.username ?? null,
      avatarUrl: discordAvatarUrl(discordUser.id, discordUser.avatar), accessTokenEncrypted: encrypt(token.access_token!),
  ...
  return { account, user, authToken, emailRequired: !user.emailVerifiedAt };
}
