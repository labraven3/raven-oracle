...
function decrypt(value: string) { const [ivRaw, tagRaw, encryptedRaw] = value.split("."); if (!ivRaw || !tagRaw || !encryptedRaw) throw new Error("Invalid encrypted value"); const decipher = crypto.createDecipheriv("aes-256-gcm", encryptionKey(), Buffer.from(ivRaw, "base64url")); decipher.setAuthTag(Buffer.from(tagRaw, "base64url")); return Buffer.concat([decipher.update(Buffer.from(encryptedRaw, "base64url")), decipher.final()]).toString("utf8"); }
...
function encryptState(payload: { userId: string | null; createdAt: number; nonce: string; returnTo: string | null }) { return encrypt(JSON.stringify(payload)); }
function decryptState(state: string) { const parsed = JSON.parse(decrypt(state)) as { userId: string | null; createdAt: number; nonce: string; returnTo?: string | null }; if (!parsed.createdAt || !parsed.nonce || parsed.createdAt + 10 * 60 * 1000 < Date.now()) throw new Error("Discord OAuth state expired"); return { ...parsed, returnTo: safeReturnTo(parsed.returnTo) }; }
function validateDiscordOAuthConfig() { if (!env.DISCORD_CLIENT_ID || !env.DISCORD_CLIENT_SECRET || !env.DISCORD_REDIRECT_URI) throw new Error("Discord authentication is not available."); }
export function createDiscordAuthorizationUrl(userId: string | null = null, returnTo: string | null = null) { validateDiscordOAuthConfig(); const state = encryptState({ userId, createdAt: Date.now(), nonce: crypto.randomBytes(24).toString("base64url"), returnTo: safeReturnTo(returnTo) }); const params = new URLSearchParams({ client_id: env.DISCORD_CLIENT_ID!, redirect_uri: env.DISCORD_REDIRECT_URI!, response_type: "code", scope: SCOPES.join(" "), state }); return `${DISCORD_AUTHORIZE_URL}?${params.toString()}`; }
