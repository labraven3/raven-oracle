...
export async function ensureChainStore() {
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS "ChainConfig" (
      "id" TEXT PRIMARY KEY,
  ...
  if (Number(countRows[0]?.count ?? 0) === 0) {
    for (const [index, name] of DEFAULT_CHAINS.entries()) {
      await prisma.$executeRawUnsafe(
        `INSERT INTO "ChainConfig" ("id", "name", "slug", "isActive", "sortOrder") VALUES (gen_random_uuid()::text, $1, $2, TRUE, $3) ON CONFLICT ("name") DO NOTHING`,
        name,
        slugify(name),
        index,
      );
...
export async function getActiveChains() {
  ...
  return prisma.$queryRawUnsafe<Array<{ id: string; name: string; slug: string; isActive: boolean; sortOrder: number }>>(
    `SELECT "id", "name", "slug", "isActive", "sortOrder" FROM "ChainConfig" WHERE "isActive" = TRUE ORDER BY "sortOrder" ASC, "name" ASC`,
  );
}
...
export async function getAllChains() {
  await ensureChainStore();
  return prisma.$queryRawUnsafe<Array<{ id: string; name: string; slug: string; isActive: boolean; sortOrder: number }>>(
    `SELECT "id", "name", "slug", "isActive", "sortOrder" FROM "ChainConfig" ORDER BY "sortOrder" ASC, "name" ASC`,
  );
}
...
export async function chainExists(name: string) {
  ...
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string }>>(
    `SELECT "id" FROM "ChainConfig" WHERE "name" = $1 AND "isActive" = TRUE LIMIT 1`,
    name,
  );
