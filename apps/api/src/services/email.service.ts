...
export async function sendEmailVerification(email: string, verificationUrl: string) {
  ...
  try {
    ...
    const body = [
      `From: ${escapeHeader(env.EMAIL_FROM_NAME)} <${env.GMAIL_USER}>`, `To: ${escapeHeader(email)}`, "Subject: Verify your Raven Oracle email", "MIME-Version: 1.0", "Content-Type: text/html; charset=UTF-8", "",
