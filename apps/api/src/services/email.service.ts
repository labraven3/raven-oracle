...
function requireGmail() {
  if (!env.GMAIL_USER || !env.GMAIL_APP_PASSWORD) throw new Error("Gmail delivery is not configured. Add GMAIL_USER and GMAIL_APP_PASSWORD to the API environment.");
}
...
export function sendEmailVerification(email: string, verificationUrl: string) {
  return sendHtmlEmail(email, "Verify your Raven Oracle email", `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px"><h1>Verify your Raven Oracle email</h1><p>Click the button below to verify your email and finish setting up your account.</p><p><a href="${verificationUrl}" style="display:inline-block;padding:12px 18px;background:#8b5cf6;color:#fff;text-decoration:none;border-radius:8px">Verify email</a></p><p style="color:#777">This link expires in 30 minutes.</p><p style="color:#777">If you did not request this, you can ignore this email.</p></div>`);
}

export function sendWinnerNotification(email: string, raffleTitle: string, prizeName: string, claimUrl: string) {
  return sendHtmlEmail(email, `You won: ${raffleTitle}`, `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px"><h1>You won a Raven Oracle raffle 🎉</h1><p>You were selected as a winner for <strong>${escapeHtml(raffleTitle)}</strong>.</p><p>Prize: <strong>${escapeHtml(prizeName)}</strong></p><p><a href="${claimUrl}" style="display:inline-block;padding:12px 18px;background:#8b5cf6;color:#fff;text-decoration:none;border-radius:8px">Open winner center</a></p><p style="color:#777">Your claim window is shown inside Raven Oracle.</p></div>`);
}
