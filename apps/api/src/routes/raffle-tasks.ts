...
router.post("/:raffleId/tasks", requireAuth, async (req, res, next) => {
  try {
    ...
    const raffle = await prisma.raffle.findUnique({ where: { id: raffleId }, select: { id: true, createdByUserId: true } }); if (!raffle) return res.status(404).json({ success: false, message: "Raffle not found" }); if (raffle.createdByUserId !== req.userId) return res.status(403).json({ success: false, message: "Only the raffle creator can add tasks" });
...
router.patch("/:raffleId/tasks/:taskId", requireAuth, async (req, res, next) => {
  try {
    ...
    const task = await prisma.raffleTask.findUnique({ where: { id: taskId }, include: { raffle: { select: { createdByUserId: true } } } }); if (!task || task.raffleId !== raffleId) return res.status(404).json({ success: false, message: "Raffle task not found" }); if (task.raffle.createdByUserId !== req.userId) return res.status(403).json({ success: false, message: "Only the raffle creator can update tasks" });
