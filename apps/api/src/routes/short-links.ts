...
function slugify(value: string) {
  return value.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 70) || "raffle";
}
...
/** Automatically attaches a short link to every newly-created project raffle response. */
...
async function redirectShortLink(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const hash = crypto.createHash("sha256").update(`${salt}:${ip}:${ua ?? "unknown"}`).digest("hex");
    await prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRaw`INSERT INTO "RaffleShortLinkClick" ("id","shortLinkId","referrer","userAgent","deviceType") VALUES (${crypto.randomUUID()}::uuid,${link.id}::uuid,LEFT(${referrer},2048),LEFT(${ua},512),${/bot|crawler|spider|preview/i.test(ua ?? "") ? "bot" : /mobile|android|iphone|ipad/i.test(ua ?? "") ? "mobile" : "desktop"})`;
