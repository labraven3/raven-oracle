...
async function requireAdmin(req: Request, res: Response, next: NextFunction) {
  ...
  if (!user || user.status === "BANNED" || !["ADMIN", "MODERATOR"].includes(user.role)) return res.status(403).json({ success: false, message: "Admin access required" });
  if (!user.isAdminApproved) return res.status(403).json({ success: false, message: "Admin access pending approval. Please contact the administrator." });
  next();
}

router.use((req, res, next) => requireAuth(req, res, next, "admin"), requireAdmin);

const createSchema = z.object({ name: z.string().trim().min(1).max(80), sortOrder: z.number().int().min(0).max(10000).optional() });

router.get("/", async (_req, res, next) => {
  try { const chains = await getAllChains(); res.json({ success: true, chains }); } catch (error) { next(error); }
});
...
router.post("/", async (req, res, next) => {
  try {
    ...
    const existing = await prisma.$queryRawUnsafe<Array<{ id: string; isActive: boolean }>>(`SELECT "id", "isActive" FROM "ChainConfig" WHERE LOWER("name") = LOWER($1) OR "slug" = $2 LIMIT 1`, parsed.data.name, slug);
...
router.patch("/:id", async (req, res, next) => {
  try {
    ...
    sets.push(`"updatedAt" = CURRENT_TIMESTAMP`); values.push(req.params.id);
    const rows = await prisma.$queryRawUnsafe<Array<{ id: string; name: string; slug: string; isActive: boolean; sortOrder: number }>>(`UPDATE "ChainConfig" SET ${sets.join(", ")} WHERE "id" = $${index} RETURNING "id", "name", "slug", "isActive", "sortOrder"`, ...values);
